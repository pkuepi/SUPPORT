...
total_male_num <- 48153604  # Total male number in North
#======================== Input path(edit) ========================#
...
individual_data <- individual_data %>%
  ...
  arrange(Agegrp) %>%
   # North ID 3e9
  mutate(
    ID = ID <- as.numeric(row_number() + 3e9 + total_male_num)) %>%
...
# ======================== I/O paths ======================== #
root_dir <- "/Generation"
in_rda   <- file.path(root_dir, "North_Female_individual_SBP_Diabetes_data.rda")
out_rda  <- file.path(root_dir, "North_SBP_Diabetes_Female.rda")
...
# ======================== 5) Save RDA ======================== #
obj_name <- "North_SBP_Diabetes_Female"
assign(obj_name, df)
