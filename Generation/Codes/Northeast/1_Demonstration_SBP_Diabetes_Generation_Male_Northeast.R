...
individual_data <- individual_data %>%
  ...
  arrange(Agegrp) %>%
   # Northeast ID 4e9
  mutate(
    ID = ID <- as.numeric(row_number() + 4e9)) %>%
...
# ======================== I/O paths ======================== #
root_dir <- "/Generation"
in_rda   <- file.path(root_dir, "Northeast_Male_individual_SBP_Diabetes_data.rda")
out_rda  <- file.path(root_dir, "Northeast_SBP_Diabetes_Male.rda")
...
# ======================== 5) Save RDA ======================== #
obj_name <- "Northeast_SBP_Diabetes_Male"
assign(obj_name, df)
