...
region <- "Northeast"   # region name
sex <- "Female"         # "Male" or "Female"
total_male_num <- 32157494  # Total male number in Northeast
#======================== Input path(edit) ========================#
...
individual_data <- individual_data %>%
  ...
  mutate(
    ...
    # Region label
    Region = labelled(4, labels = c(Central = 1, East = 2, North = 3, Northeast = 4, Northwest = 5, South = 6, Southwest = 7)),
  ) %>% 
  arrange(Agegrp) %>%
   # Northeast ID 4e9
  mutate(
    ID = ID <- as.numeric(row_number() + 4e9 + total_male_num)) %>%
...
individual_data <- individual_data %>%
  mutate(
    Sex = labelled(2, labels = c(Male = 1, Female = 2))
  )
...
# ======================== I/O paths ======================== #
root_dir <- "/Generation"
in_rda   <- file.path(root_dir, "Northeast_Female_individual_SBP_Diabetes_data.rda")
out_rda  <- file.path(root_dir, "Northeast_SBP_Diabetes_Female.rda")
...
cat("==============================================================================\n")
cat("(Female): keep ONLY original names; 20 groups = Age group x Sex\n")
...
# ======================== 5) Save RDA ======================== #
obj_name <- "Northeast_SBP_Diabetes_Female"
assign(obj_name, df)
