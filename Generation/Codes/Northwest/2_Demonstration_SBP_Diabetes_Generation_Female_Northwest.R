...
region <- "Northwest"   # region name
sex <- "Female"         # "Male" or "Female"
total_male_num <- 27644285  # Total male number in Northwest
#======================== Input path(edit) ========================#
...
individual_data <- individual_data %>%
  ...
  mutate(
    ...
    # Region label
    Region = labelled(5, labels = c(Central = 1, East = 2, North = 3, Northeast = 4, Northwest = 5, South = 6, Southwest = 7)),
  ) %>% 
  arrange(Agegrp) %>%
   # Northwest ID 5e9
  mutate(
    ID = ID <- as.numeric(row_number() + 5e9 + total_male_num)) %>%
...
# ======================== I/O paths ======================== #
root_dir <- "/Generation"
in_rda   <- file.path(root_dir, "Northwest_Female_individual_SBP_Diabetes_data.rda")
out_rda  <- file.path(root_dir, "Northwest_SBP_Diabetes_Female.rda")
...
# ======================== 5) Save RDA ======================== #
obj_name <- "Northwest_SBP_Diabetes_Female"
assign(obj_name, df)
