...
individual_data <- individual_data %>%
  ...
  arrange(Agegrp) %>%
   # Northwest ID 5e9
  mutate(
    ID = ID <- as.numeric(row_number() + 5e9)) %>%
...
# ======================== I/O paths ======================== #
root_dir <- "/Generation"
in_rda   <- file.path(root_dir, "Northwest_Male_individual_SBP_Diabetes_data.rda")
out_rda  <- file.path(root_dir, "Northwest_SBP_Diabetes_Male.rda")
...
# ======================== 5) Save RDA ======================== #
obj_name <- "Northwest_SBP_Diabetes_Male"
assign(obj_name, df)
