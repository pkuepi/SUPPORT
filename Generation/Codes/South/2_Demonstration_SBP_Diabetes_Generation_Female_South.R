...
region <- "South"   # region name
sex <- "Female"         # "Male" or "Female"
total_male_num <- 46766167  # Total male number in South
#======================== Input path(edit) ========================#
...
individual_data <- individual_data %>%
  ...
  mutate(
    ...
    # Region label
    Region = labelled(6, labels = c(Central = 1, East = 2, North = 3, Northeast = 4, Northwest = 5, South = 6, Southwest = 7)),
  ) %>% 
  arrange(Agegrp) %>%
   # South ID 6e9
  mutate(
    ID = ID <- as.numeric(row_number() + 6e9 + total_male_num)) %>%
...
# ======================== I/O paths ======================== #
root_dir <- "/Generation"
in_rda   <- file.path(root_dir, "South_Female_individual_SBP_Diabetes_data.rda")
out_rda  <- file.path(root_dir, "South_SBP_Diabetes_Female.rda")
...
# ======================== 5) Save RDA ======================== #
obj_name <- "South_SBP_Diabetes_Female"
assign(obj_name, df)
