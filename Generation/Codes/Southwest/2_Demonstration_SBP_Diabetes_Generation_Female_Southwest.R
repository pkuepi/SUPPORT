...
region <- "Southwest"   # region name
sex <- "Female"         # "Male" or "Female"
total_male_num <- 56033689  # Total male number in Southwest
#======================== Input path(edit) ========================#
...
individual_data <- individual_data %>%
  ...
  mutate(
    ...
    # Region label
    Region = labelled(7, labels = c(Central = 1, East = 2, North = 3, Northeast = 4, Northwest = 5, South = 6, Southwest = 7)),
  ) %>% 
  arrange(Agegrp) %>%
   # Southwest ID 7e9
  mutate(
    ID = ID <- as.numeric(row_number() + 7e9 + total_male_num)) %>%
...
# ======================== I/O paths ======================== #
root_dir <- "/Generation"
in_rda   <- file.path(root_dir, "Southwest_Female_individual_SBP_Diabetes_data.rda")
out_rda  <- file.path(root_dir, "Southwest_SBP_Diabetes_Female.rda")
...
# ======================== 5) Save RDA ======================== #
obj_name <- "Southwest_SBP_Diabetes_Female"
assign(obj_name, df)
