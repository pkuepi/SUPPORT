...
total_male_num <- 59812000  # Total male number in Central
#======================== Input path(edit) ========================#
...
individual_data <- individual_data %>%
  ...
  mutate(
    ...
    # Region label
    Region = labelled(1, labels = c(Central = 1, East = 2, North = 3, Northeast = 4, Northwest = 5, South = 6, Southwest = 7)),
  ) %>% 
  arrange(Agegrp) %>%
   # Central ID 1e9
  mutate(
    ID = ID <- as.numeric(row_number() + 1e9 + total_male_num)) %>%
...
# ======================== I/O paths ======================== #
root_dir <- "/Generation"
in_rda   <- file.path(root_dir, "Central_Female_individual_SBP_Diabetes_data.rda")
out_rda  <- file.path(root_dir, "Central_SBP_Diabetes_Female.rda")
...
# ======================== 5) Save RDA ======================== #
obj_name <- "Central_SBP_Diabetes_Female"
assign(obj_name, df)
